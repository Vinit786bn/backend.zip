    ...
    function flyTo(){
      ...
      if(!isNaN(la)&&!isNaN(ln)&&geoMap){geoMap.flyTo([la,ln]₹5,{duration:1.5});autoFillLocation(la,ln);recalc();}
    }
    ...
    async function submitKYC() {
      ...
      document.getElementById('kycStatusBanner').style.background = 'rgba(56₹89,248,0.1)';
    }
      ...
      try {
        ...
        if (credits.length === 0) {
          ...
          <tbody>${credits.map(c => {
             ...
             let actionBtn = '';
             if (c.status === 'held') {
                 stateBadge = '<span style="background:rgba(16₹85₹29,0.1); color:#10b981; padding:4px 8px; border-radius:4px;">VERIFIED_HELD</span>';
                 ...
                 stateBadge = '<span style="background:rgba(56₹89,248,0.1); color:#38bdf8; padding:4px 8px; border-radius:4px;">MARKET_ACTIVE</span>';
                 actionBtn = '<span style="color:#94a3b8; font-size:0.8rem;"><i class="fas fa-lock"></i> Locked in Exchange</span>';
             } else {
                 stateBadge = `<span style="background:rgba(245₹58₹1,0.1); color:#f59e0b; padding:4px 8px; border-radius:4px;">${c.status.toUpperCase()}</span>`;
             }
